...
def get_dwt_rgb(self, img):
  ...
  # cA_h, (cH_h, cV_h, cD_h) = pywt.dwt2(h, self.w, mode=self.m)
  # cA_s, (cH_s, cV_s, cD_s) = pywt.dwt2(s, self.w, mode=self.m)
  # cA_v, (cH_v, cV_v, cD_v) = pywt.dwt2(v, self.w, mode=self.m)

  # cA_h = cv2.resize(cA_h, self.image_size, interpolation=cv2.INTER_LINEAR).astype('float32')
  # cA_s = cv2.resize(cA_s, self.image_size, interpolation=cv2.INTER_LINEAR).astype('float32')
  # cA_v = cv2.resize(cA_v, self.image_size, interpolation=cv2.INTER_LINEAR).astype('float32')
